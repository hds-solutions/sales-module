    ...
    <div class="card-header">
        <div class="row">
            <div class="col-6 d-flex align-items-center">
                ...
                @lang('sales::invoices.purchases.show')
            </div>
            <div class="col-6 d-flex justify-content-end">
                {{-- @if ($resource->isCompleted())
                <button class="btn btn-sm btn-outline-info"
                    data-printable="{{ route('backend.purchases.invoices.print', $resource) }}" data-print="true">
                    <i class="fas fa-print"></i>
                </button>
                @endif --}}

                @if (!$resource->wasCompleted())
                <a href="{{ route('backend.purchases.invoices.edit', $resource) }}"
                    class="btn btn-sm ml-2 btn-outline-secondary btn-hover-info">@lang('sales::invoices.purchases.edit')</a>
                @endif
                <a href="{{ route('backend.purchases.invoices.create') }}"
                    class="btn btn-sm ml-2 btn-outline-primary">@lang('sales::invoices.purchases.create')</a>
            </div>
        </div>
    </div>
    <div class="card-body">
        ...
        <div class="row">
            <div class="col-12 col-xl-6">
                ...
                <div class="row">
                    <div class="col-4 col-lg-4">@lang('sales::invoice.provider_id.0'):</div>
                ...
                @if ($resource->wasCompleted())
                <div class="row">
                    <div class="col-4 col-lg-4">@lang('sales::invoice.in_out_id.0'):</div>
                    <div class="col-8 col-lg-6 h4"><a href="{{ route('backend.purchases.in_outs.show', $resource->inOut) }}"
                        class="text-decoration-none text-muted">{{ $resource->inOut?->document_number }}</a></div>
                </div>
                @endif
                            ...
                            @foreach ($resource->lines as $line)
                                ...
                                <tr class="collapse line-{{ $line->id }}-details">
                                    ...
                                    <td class="py-0 pl-3" colspan="3">
                                        <a href="{{ route('backend.purchases.orders.show', $orderLine->order) }}"
        ...
        @include('backend::components.document-actions', [
            'route'     => 'backend.purchases.invoices.process',
            'resource'  => $resource,
        ])
